    ...
    <div className="question-solver-page">
      ...
      <div id="pdf-export-wrapper" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', width: '100%' }}>
        ...
        <div className="qs-results-grid">
          ...
          {/* Right Column: Diagram + Table (Diagram excluded from PDF) */}
          <div className="qs-right-column">
            {status === 'solving' ? (
              <div className="qs-solving-indicator" data-html2canvas-ignore>
                <PencilLoader />
                <p style={{ marginTop: '1rem', color: 'var(--color-primary-light)', fontWeight: 600 }}>Analyzing & Generating Automaton...</p>
              </div>
            ) : (
              <>
                <div data-html2canvas-ignore style={{ minHeight: '400px', background: 'var(--color-bg-card)', border: '1px solid var(--color-border)', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--color-text-muted)' }}>
                  <p>Interactive diagram (view in app)</p>
                </div>
                <SolutionDiagram style={{ display: 'none' }} />
                <TransitionTableView />
              </>
            )}
          </div>
