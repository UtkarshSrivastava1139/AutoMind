  ...
  return (
    <div className="test-case-panel glass-card">
      <h3 className="panel-title" style={{ display: 'flex', alignItems: 'center' }}>
        <TestTubes className="text-secondary mr-2" size={20} style={{ marginRight: '8px' }} />
        Test Cases
        ...
        <div className="test-section">
          ...
          <div className="test-list">
            {positiveTests.map((t, i) => (
              <div key={i} className={`test-item ${t.passed ? 'test-pass' : 'test-fail'}`} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <span className="test-icon" style={{ display: 'flex', alignItems: 'center' }}>
                  {t.passed ? <Check size={16} /> : <X size={16} />}
                </span>
      ...
      {counterexamples.length > 0 && (
        <div className="test-section">
          <h4 className="test-section-title counterexample-title" style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
            <AlertOctagon size={16} /> Counterexamples
          </h4>
          <div className="test-list">
            {counterexamples.map((c, i) => (
              <div key={i} className="test-item test-fail" style={{ display: 'flex', alignItems: 'flex-start', gap: '8px' }}>
                <span className="test-icon" style={{ marginTop: '2px' }}><AlertOctagon size={14} /></span>
