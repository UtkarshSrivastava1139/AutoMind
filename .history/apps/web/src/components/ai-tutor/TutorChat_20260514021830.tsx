                ...
                {msg.role === 'user' ? (
                  ...
                  <div className="prose prose-invert max-w-none text-sm leading-relaxed">
                    <ReactMarkdown 
                      ...
                      components={{
                        h2: ({node, ...props}) => <h4 className="font-bold text-lg mt-4 mb-2 text-slate-100" {...props} />,
                        h3: ({node, ...props}) => <h5 className="font-bold text-base mt-3 mb-2 text-slate-200" {...props} />,
                        p: ({node, ...props}) => <p className="mb-2" {...props} />,
                        li: ({node, ...props}) => <li className="mb-1 ml-4 list-disc" {...props} />,
                        ul: ({node, ...props}) => <ul className="mb-3" {...props} />,
                        ol: ({node, ...props}) => <ol className="mb-3 list-decimal ml-4" {...props} />,
                        code: ({node, ...props}) => <code className="bg-slate-900/50 px-1.5 py-0.5 rounded text-blue-300 font-mono text-xs" {...props} />,
                        pre: ({node, ...props}) => <pre className="bg-slate-900/80 p-3 rounded-lg my-3 overflow-x-auto text-xs" {...props} />
                      }}
                    >
                      {msg.content}
                    </ReactMarkdown>
